 ...
 */
package com.msharp.sharding.jdbc.single.pool;

import com.msharp.sharding.jdbc.exception.MSharpConfigException;
import com.msharp.sharding.jdbc.single.config.DataSourceConfig;
import com.msharp.sharding.jdbc.enums.DataSourceState;
...
import com.msharp.sharding.jdbc.util.JdbcDriverClassHelper;
import com.msharp.sharding.jdbc.util.StringUtils;
...
public class Dbcp2DataSourcePool extends AbstractDataSourcePool implements DataSourcePool {
	...
	public DataSource build(DataSourceConfig config, boolean withDefaultValue) {
		final BasicDataSource dbcp2DataSource = new BasicDataSource();
		...
		dbcp2DataSource.setDriverClassName(StringUtils.isNotBlank(config.getDriverClass()) ? config.getDriverClass() : JdbcDriverClassHelper.getDriverClassNameByJdbcUrl(config.getJdbcUrl()));
		...
		if (withDefaultValue) {
			...
			dbcp2DataSource.setTimeBetweenEvictionRunsMillis(getIntProperty(config, "timeBetweenEvictionRunsMillis", 30000)); // 30s
			...
			try {
				...
				throw new MSharpConfigException(String.format("dbcp2 dataSource [%s] created error : ", config.getId()), e);
			}
		}
